...
def run_test_draw_points_on_circle():
    ...
    title = 'Test 4:  4 blue dots.'
    window = rg.RoseWindow(400, 440, title)
    circle = rg.Circle(rg.Point(100, 50), 25)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(generate_points_on_circle(circle, number_of_points))):
        circles_on_points = rg.Circle(generate_points_on_circle(circle, number_of_points)[k], 10)
        circles_on_points.fill_color = color
        circles_on_points.attach_to(window)
        generate_points_on_circle(circle, number_of_points)[k].attach_to(window)
...
def run_test_pizza():
    ...
    circle.outline_thickness = 3
    pizza(window, circle, 70, 'black', 4)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON test 3:  3 segments with green lines.'
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(150, 100), 50)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(seq)):
        value = (k + hops_to_next_point) % len(seq)
        line = rg.Line(seq[k], seq[value])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
        line.arrow = 'last'
